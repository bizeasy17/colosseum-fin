...
from chartsnfigures.models import CNFReport, Category, Tag, Links, SideBar
from django.utils.encoding import force_text
from django.shortcuts import get_object_or_404
import hashlib
import urllib
from comments.models import Comment
from colosseum.utils import cache_decorator, cache
...
def custom_markdown(content):
    from colosseum.utils import CommonMarkdown
...
@register.inclusion_tag('chartsnfigures/tags/breadcrumb.html')
def load_breadcrumb(cnfreport):
    ...
    return {
        'names': names,
        'title': cnfreport.title
    }


@register.inclusion_tag('chartsnfigures/tags/report_tag_list.html')
def load_articletags(cnfreport):
    """
    文章标签
    :param charts & figures report:
    :return:
    """
    tags = cnfreport.tags.all()
    tags_list = []
    for tag in tags:
        url = tag.get_absolute_url()
        count = tag.get_report_count()
        tags_list.append((
            url, count, tag, random.choice(settings.BOOTSTRAP_COLOR_TYPES)
        ))
    return {
        'cnfreport_tags_list': tags_list
    }


@register.inclusion_tag('chartsnfigures/tags/sidebar.html')
def load_sidebar(user, linktype):
    ...
    from colosseum.utils import get_blog_setting
    blogsetting = get_blog_setting()
    recent_cnfreports = CNFReport.objects.filter(status='p')[:blogsetting.sidebar_report_count]
    sidebar_categorys = Category.objects.all()
    extra_sidebars = SideBar.objects.filter(is_enable=True).order_by('sequence')
    most_read_cnfreports = CNFReport.objects.filter(status='p').order_by('-views')[:blogsetting.sidebar_report_count]
...
@register.inclusion_tag('chartsnfigures/tags/report_meta_info.html')
def load_report_metas(cnfreport, user):
    """
    获得文章meta信息
    :param charts figures report:
    :return:
...
@register.inclusion_tag('chartsnfigures/tags/report_pagination.html')
...
@register.inclusion_tag('chartsnfigures/tags/report_info.html')
