...
def get_max_articleid_commentid():
    ...
    return (CNFReport.objects.latest().pk, Comment.objects.latest().pk)
...
class CommonMarkdown():
    ...
    def get_markdown(value):
        renderer = MarkDownRenderer(inlinestyles=False)
...
def send_email(emailto, title, content):
    from colosseum.my_signals import send_email_signal
...
def get_blog_setting():
    ...
    if value:
        ...
        from chartsnfigures.models import WebsiteSettings
        if not WebsiteSettings.objects.count():
            setting = WebsiteSettings()
            setting.sitename = _('Colosseum FinTech')
            setting.site_description = _('基于Django的金融图表和交易记录平台')
            setting.site_seo_description = _('基于Django的金融图表和交易记录平台')
            setting.site_keywords = _('Django,Python,Trade,Finance,Chart')
            setting.cnfreport_sub_length = 300
            setting.sidebar_cnfreport_count = 10
            ...
            setting.save()
        value = WebsiteSettings.objects.first()
...
def delete_sidebar_cache(username):
    ...
    from chartsnfigures.models import LINK_SHOW_TYPE
